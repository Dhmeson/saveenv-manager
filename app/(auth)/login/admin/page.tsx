  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res?.ok) {
        ...
        // Isso será verificado no servidor, mas podemos fazer uma verificação adicional aqui
        startTransition(() => {
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center p-6">
        <div className="w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 text-white/90 shadow-2xl backdrop-blur">
          <div className="mb-6 text-center">
            ...
            <h1 className="bg-gradient-to-br from-white to-white/70 bg-clip-text text-2xl font-extrabold tracking-tight text-transparent">Admin Login</h1>
            <p className="text-sm text-white/70">Sign in as administrator</p>
          </div>
          ...
          <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); handleLogin() }}>
            <div>
              ...
              <div className="relative">
                <Shield className="absolute left-3 top-2.5 h-5 w-5 text-white/60" />
                <input
                  type="email"
                  name="email"
                  value={email}
            ...
            >
              {loading || pending ? 'Signing in…' : 'Sign in as Admin'}
            </button>

            <div className="flex items-center justify-between text-sm">
              <Link href="/login" className="text-white/70 underline-offset-4 hover:text-white hover:underline">
                User Login
              </Link>
